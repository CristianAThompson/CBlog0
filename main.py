...
# Define the base Handler class that allows quick usage of the
# webapp2.requestHandler also defines some base methods and check and validates
# the cookie
...
# Create a secure tuple of a passed in string of the string and hmac
...
# Splits the passed in value and takes the first index to compare against
# itself passed in to make_secure function
...
# to create a hashed password of the username password and salt passed encoded
# in sha256 if no salt is declared, or present in user information run
# make_salt to create one
...
# Checks the hashed password against the stored user credentials splits the h
# value and uses index location 1 as a variable to compare against for
...
# Dictates where a user is directed if they are signed in or not and accesses
# the page from the base url
...
# Defines the User Entity and several functions specifically tied to User
# entities
...
class User(db.Model):
    ...
    # Retreives a user from the User database where the username is the passed
    # in username
    @classmethod
    ...
    # Verifies that the passed in username and the stored hashed password are
    # True then logs in
    @classmethod
...
# Defines the default page and if user is signed in retrieve that users
# blog posts
...
class PermaHandler(Handler):
    ...
    # Retrieves and manages the content displayed on the permalink page based
    # upon what user is accessing the page
    def get(self, post_id):
        # Retrieve the specific Blog entry and all comments for that entry with
        # the post_id from the url
        ...
        if self.user:
            ...
            self.render('perma.html', post=post, username=self.user.username,
                        comments=comments, post_owner=post_owner)
        else:
            no_user = "Please Sign In To Comment or Like this post!"
            self.render(
                'perma.html', post=post, comments=comments, no_user=no_user)

    # Defines the functionality of the comment submission form for users who
    ...
    def post(self, post_id):
        ...
        if self.user:
            ...
            if not post:
                self.render('404.html')
                return
            # If content is present and is a signed in user then creates new
            # comment entity with the user submitted information
            if content and self.user:
                comment = Comment(comment_username=username,
                                  comment_content=content, post_id=int(
                                      post_id),
                                  parent_id=int(post.key().id()))
                comment.put()
                time.sleep(.3)
                self.redirect('/id=%s' % str(post.key().id()))
            else:
                cerror = "Make sure you added a comment before you submitted."
                self.render('perma.html', post=post, username=username,
                            comments=comments, comment_error=cerror)
        else:
            self.redirect('/id=%s' % str(post.key().id()))
        ...
        if self.user:
            ...
            if subject and content:
                b = Blog(subject=subject, content=content,
                         submitted_user=make_secure(self.user.username))
                b.put()
...
# Tests taking in submitted user data and comparing it against regular
# expressions
...
class SignUpHandler(Handler):
    ...
    def post(self):
        ...
        # Retrieve the user submitted data and a value to compare against for
        # validation
        error = False
        ...
        # If not a valide username against the regular expression define error
        # and an error parameter
        if not valid_username(self.username):
            params[
                'nerror'
            ] = "You entered an invalid Username, please try again."
            error = True

        # If not a valide password against the regular expression define error
        # and an error parameter
        if not valid_password(self.password):
            params[
                'perror'] = "You typed an invalid password, please try again."
            error = True

        # If the user submitted password doesn't match the second password
        # define error and an error parameter
        ...
        # If error is present then re-render the sign-up form with the
        # parameters or else run the function done()
        if error:
...
class RegisterPage(SignUpHandler):
    ...
    def done(self):
        ...
        # If there is a user with that username raise an error and re-render
        # form
        if u:
            exists = 'That user already exists.'
            self.render('sign-up.html', nerror=exists)
        # Take user submitted data and format it for the User entity, place the
        # formatted data inside the User entity and redirect to the welcome
        # page
        else:
...
# Takes in Handler and renders a login form, then retrieves that data after
# submission and checks if the username and password are correct, if correct
# redirect to the blog, if not correct re-render login form with an error
...
# Takes Handler renders an edit page if the user signed in is the same user
...
class EditPage(Handler):
    ...
    def get(self, post_id):
        ...
        # Retrieve the specific blog entry from the post_id which maps to the
        # id integer in the url
        key = db.Key.from_path('Blog', int(post_id))
        post = db.get(key)

        # Verify that the submitted user and the currently signed in user are
        # the same if they are render an edit form with the content of the post
        # passed in, if they don't match redirect to the perma-link of the post
        if not post:
            self.render('404.html')
            return
        if self.user:
            if post.submitted_user == make_secure(self.user.username):
                self.render(
                    'edit.html', post=post, username=self.user.username)
        else:
            self.redirect('/id=%s' % str(post.key().id()))

    # Capture the form submission from the edit page and resubmit the blog
    ...
    def post(self, post_id):
        ...
        if self.user:
            if post.submitted_user == make_secure(self.user.username):
                ...
                # If both subject and content are present re-define the current
                # post content with the newly submitted content and place it
                # back within the Blog entity then redirect to the perma-link
                if subject and content:
                    post.content = content
                    post.put()
                    time.sleep(.2)
                    self.redirect('/id=%s' % str(post.key().id()))
                # If both subject and content aren't present define an error
                # and re-render the form with that error
                else:
                    error = "Make sure you included both fields."
...
class EditComment(Handler):
    ...
    def get(self, post_id):
        ...
        # If there is a user and that user is the person who created the
        # comment render the page with the comment information inside the form
        # fields if not redirect to the original post
        if self.user:
            if post.comment_username == self.user.username:
                self.render(
                    'editcomment.html', post=post, username=self.user.username)
            else:
...
# Retrieve the specific blog post by ID and if the submitted user and the
# hashed currently logged in user match then use the built in delete function
# on the entity and redirct to the main blog page
...
# logged in user match then use the built in delete to remove the entry and
# redirect back to the orignal post
...
class LikePost(Handler):
    ...
    def get(self, post_id):
        ...
        # likes then add current user to the likes entry and add one to the
        # likes total if the user has already liked the post then remove name
        # from likes and reduce the likes total by 1
        if self.user:
            if post.likes:
                if make_secure(self.user.username) != post.submitted_user:
                    if self.user.username not in post.likes:
                        ...
                        post.likes = str(post.likes).replace(
                            self.user.username, "")
                        post.like_total -= 1
                        post.put()
            # If there are no likes present and the current user isn't the post
            # author then create likes with the current username and create
            # likes total with a value of 1
            else:
                if make_secure(self.user.username) != post.submitted_user:
                    post.likes = self.user.username
                    post.like_total = 1
